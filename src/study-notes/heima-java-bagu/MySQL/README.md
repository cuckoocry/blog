...
对于复合索引(多列索引)，查询条件没有从索引的最左列开始

- 1. 例如索引是(a,b,c)，但查询只使用b和c  (跳过最左列（完全失效）)

- 2. 范围查询后停止（范围查询列及其左侧列能使用索引）
```sql 
WHERE a = 1 AND b > 2 AND c = 3
-- 只能用到a和b的索引，c不能
```
- 3. 中间有列缺失（只能用到缺失列左侧的部分）
```sql
WHERE a = 1 AND c = 3
-- 只能用a的索引，c不能
```
...
2. 范围查询右边的列，不能使用索引
3. 在索引列上使用函数或运算符
4. 字符串不加单引号，造成索引失效。(隐式类型转换会导致索引失效，如字符串列与数字比较：WHERE string_column = 123)
5. 以`%`号开头的`like`模糊查询，会导致索引失效
6. 使用`ORDER BY`与索引顺序不一致,当`ORDER BY`子句与索引顺序不一致时，可能导致索引失效
7. 当OR条件中有一列没有索引时，整个查询可能无法使用索引
8. 使用不等于(!=或<>)操作符,`NOT IN`,使用`IS NULL`或`IS NOT NULL`
