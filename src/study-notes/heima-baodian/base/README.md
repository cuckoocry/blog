...
拿到key,拿到key的hashCode，二次hsah,与数组容量减一去按位与，计算出key的索引位置，也就是桶下标

### 2）树化与退化

红黑树：父节点左侧都比他小，右侧都是比他大的数据。
...
2. **扩容时重新计算索引效率更高**： hash & oldCap（旧容量） == 0 的元素留在原来位置 ，否则新位置 = 旧位置 + oldCap
...
2. 计算索引（桶下标）。（拿到key,拿到key的hashCode，二次hsah,与数组容量减一去按位与，计算出key的索引位置，也就是桶下标）
