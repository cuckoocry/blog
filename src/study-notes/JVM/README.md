...
1.  虚拟机栈（栈帧中的本地变量表）中的引用的对象。(比如：各个线程被调用的方法中使用到的参数、局部变量等)
2.  方法栈中JNI（`Native`方法）的引用的对象。 
3.  方法区域中常量引用的对象。(字符串常量池（string Table）里的引用、被 final 修饰的常量)
4.  方法区域中的类静态属性引用的对象。（static 修饰的变量）
